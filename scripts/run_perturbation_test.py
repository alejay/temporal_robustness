"""
Run controlled perturbation tests from existing trained checkpoints.
"""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

import torch

sys.path.insert(0, str(Path(__file__).parent.parent))

from diagnostics.perturbation_test import (
    parse_float_list,
    run_all_perturbation_tests,
    save_perturbation_outputs,
)
from experiment_registry import DATASET_REGISTRY, MODEL_REGISTRY, TASK_REGISTRY, checkpoint_path
from models.landmarking import LandmarkingModel
from tracking import (
    build_data_provenance,
    collect_git_metadata,
    default_run_name,
    log_git_tags,
    log_params_from_namespace,
    maybe_log_artifacts,
    maybe_log_metrics,
    set_common_tags,
    start_run_if_enabled,
    write_provenance_files,
)


DEFAULTS = dict(
    raw_dir="data/raw",
    results_dir="results",
    run_subdir="",
    max_patients=None,
    batch_size=32,
    hidden_dim=64,
    latent_dim=32,
    n_ode_samples=20,
    seed=42,
    models="grud,latent_ode,transformer,state_space,landmarking",
    dataset="physionet2012",
    task="mortality",
    prediction_horizon_hours=6.0,
    label_window_hours=24.0,
    task_mode="patient_binary",
    transformer_heads=4,
    transformer_layers=2,
    state_dim=32,
    landmark_grid_hours=6.0,
    n_bootstrap_samples=10,
    retention_levels="1.0,0.75,0.5,0.25,0.1",
    n_repeats=10,
    timestamp_factors="0.5,1.0,2.0,4.0",
    fast=False,
    track=False,
    tracking_uri="file:./mlruns",
    experiment_name="temporal_robustness",
    run_name="",
)


def parse_args():
    """Build the perturbation-test CLI with grouped, user-facing descriptions."""
    parser = argparse.ArgumentParser(
        description="Run learned-model perturbation tests from existing checkpoints.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    io_group = parser.add_argument_group("Paths")
    io_group.add_argument(
        "--raw_dir",
        type=str,
        default=DEFAULTS["raw_dir"],
        help="Directory containing raw dataset files or automatic downloads.",
    )
    io_group.add_argument(
        "--results_dir",
        type=str,
        default=DEFAULTS["results_dir"],
        help="Base results directory containing checkpoints and perturbation outputs.",
    )
    io_group.add_argument(
        "--run_subdir",
        type=str,
        default=DEFAULTS["run_subdir"],
        help="Optional isolated run root under results_dir for checkpoints, outputs, and provenance.",
    )

    data_group = parser.add_argument_group("Data And Task")
    data_group.add_argument(
        "--dataset",
        type=str,
        default=DEFAULTS["dataset"],
        help="Dataset adapter to load. Public v1 uses physionet2012.",
    )
    data_group.add_argument(
        "--task",
        type=str,
        default=DEFAULTS["task"],
        help="Task adapter to apply after dataset loading.",
    )
    data_group.add_argument(
        "--task_mode",
        type=str,
        default=DEFAULTS["task_mode"],
        help="Task labeling mode used by the task adapter.",
    )
    data_group.add_argument(
        "--prediction_horizon_hours",
        type=float,
        default=DEFAULTS["prediction_horizon_hours"],
        help="Prediction horizon metadata in hours for the selected task.",
    )
    data_group.add_argument(
        "--label_window_hours",
        type=float,
        default=DEFAULTS["label_window_hours"],
        help="Label window metadata in hours for the selected task.",
    )
    data_group.add_argument(
        "--max_patients",
        type=int,
        default=DEFAULTS["max_patients"],
        help="Optional cap on loaded patients, mainly for smoke or debug runs.",
    )
    data_group.add_argument(
        "--seed",
        type=int,
        default=DEFAULTS["seed"],
        help="Random seed used for thinning, prefixes, and timestamp perturbations.",
    )

    selection_group = parser.add_argument_group("Method Selection")
    selection_group.add_argument(
        "--models",
        type=str,
        default=DEFAULTS["models"],
        help="Comma-separated learned models to evaluate.",
    )
    perturb_group = parser.add_argument_group("Perturbation Tests")
    perturb_group.add_argument(
        "--retention_levels",
        type=str,
        default=DEFAULTS["retention_levels"],
        help="Comma-separated retention fractions for retrospective thinning tests.",
    )
    perturb_group.add_argument(
        "--n_repeats",
        type=int,
        default=DEFAULTS["n_repeats"],
        help="Number of repeated random thinnings per retention level.",
    )
    perturb_group.add_argument(
        "--timestamp_factors",
        type=str,
        default=DEFAULTS["timestamp_factors"],
        help="Comma-separated multiplicative factors for timestamp stretching.",
    )
    perturb_group.add_argument(
        "--fast",
        action="store_true",
        default=DEFAULTS["fast"],
        help="Run a reduced configuration for quick verification.",
    )

    model_group = parser.add_argument_group("Model Hyperparameters")
    model_group.add_argument(
        "--batch_size",
        type=int,
        default=DEFAULTS["batch_size"],
        help="Mini-batch size used during prediction loops.",
    )
    model_group.add_argument(
        "--hidden_dim",
        type=int,
        default=DEFAULTS["hidden_dim"],
        help="Shared hidden dimension for checkpoint reconstruction.",
    )
    model_group.add_argument(
        "--latent_dim",
        type=int,
        default=DEFAULTS["latent_dim"],
        help="Latent state dimension for latent ODE checkpoint reconstruction.",
    )
    model_group.add_argument(
        "--n_ode_samples",
        type=int,
        default=DEFAULTS["n_ode_samples"],
        help="Number of posterior samples for latent ODE probability estimates.",
    )
    model_group.add_argument(
        "--transformer_heads",
        type=int,
        default=DEFAULTS["transformer_heads"],
        help="Number of attention heads in transformer checkpoints.",
    )
    model_group.add_argument(
        "--transformer_layers",
        type=int,
        default=DEFAULTS["transformer_layers"],
        help="Number of transformer encoder layers in checkpoints.",
    )
    model_group.add_argument(
        "--state_dim",
        type=int,
        default=DEFAULTS["state_dim"],
        help="Latent state dimension for state-space checkpoints.",
    )
    model_group.add_argument(
        "--landmark_grid_hours",
        type=float,
        default=DEFAULTS["landmark_grid_hours"],
        help="Landmark spacing used when reconstructing landmarking checkpoints.",
    )
    model_group.add_argument(
        "--n_bootstrap_samples",
        type=int,
        default=DEFAULTS["n_bootstrap_samples"],
        help="Bootstrap ensemble count for landmarking checkpoints.",
    )

    tracking_group = parser.add_argument_group("Tracking")
    tracking_group.add_argument(
        "--track",
        action="store_true",
        default=DEFAULTS["track"],
        help="Enable optional MLflow tracking and provenance logging.",
    )
    tracking_group.add_argument(
        "--tracking_uri",
        type=str,
        default=DEFAULTS["tracking_uri"],
        help="MLflow tracking URI, typically a local file store.",
    )
    tracking_group.add_argument(
        "--experiment_name",
        type=str,
        default=DEFAULTS["experiment_name"],
        help="MLflow experiment name used when --track is enabled.",
    )
    tracking_group.add_argument(
        "--run_name",
        type=str,
        default=DEFAULTS["run_name"],
        help="Optional MLflow run name. Defaults to an auto-generated name.",
    )

    args = parser.parse_args()
    args.retention_levels = parse_float_list(args.retention_levels)
    args.timestamp_factors = parse_float_list(args.timestamp_factors)
    return args


def parse_name_list(value: str):
    """Parse a comma-separated CLI list into non-empty names."""
    return [item.strip() for item in value.split(",") if item.strip()]


def get_device():
    """Choose the preferred torch device available in the current environment."""
    if torch.cuda.is_available():
        device = torch.device("cuda")
    elif torch.backends.mps.is_available():
        device = torch.device("mps")
    else:
        device = torch.device("cpu")
    print(f"Device: {device}")
    return device


def _checkpoint_for(model_name: str, out_dir: Path) -> Path:
    """Return the expected checkpoint path for one trained learned model."""
    spec = MODEL_REGISTRY[model_name]
    return checkpoint_path(model_name, "model", out_dir, spec.checkpoint_suffix)


def load_model_checkpoint(model_name: str, cfg, meta, device, out_dir: Path):
    """Load one trained learned model checkpoint for perturbation evaluation."""
    if model_name not in MODEL_REGISTRY:
        raise ValueError(f"Unknown model '{model_name}'. Available: {sorted(MODEL_REGISTRY)}")
    path = _checkpoint_for(model_name, out_dir)
    if not path.exists():
        raise FileNotFoundError(
            f"Missing checkpoint for {model_name}: {path}. "
            "Create the checkpoint first, or use --models to select models "
            "with existing checkpoints."
        )
    if model_name == "landmarking":
        return LandmarkingModel.load(path)
    model = MODEL_REGISTRY[model_name].builder(cfg, meta, device)
    model.load_state_dict(torch.load(path, map_location=device))
    model.to(device)
    model.eval()
    return model


def main():
    """Run the learned-model perturbation test workflow."""
    cfg = parse_args()
    if cfg.fast:
        cfg.max_patients = cfg.max_patients or 300
        cfg.n_ode_samples = min(cfg.n_ode_samples, 5)
        cfg.n_repeats = min(cfg.n_repeats, 3)
        print("Fast mode: reduced patients, ODE samples, and repeats.")

    selected_models = parse_name_list(cfg.models)
    out_dir = Path(cfg.results_dir)
    if cfg.run_subdir:
        out_dir = out_dir / cfg.run_subdir
    device = get_device()

    print("\n── Loading data ────────────────────────────────────────────────")
    raw_splits, raw_regimes, meta = DATASET_REGISTRY[cfg.dataset].load(cfg)
    splits, regimes, meta = TASK_REGISTRY[cfg.task].apply(raw_splits, raw_regimes, meta, cfg)
    test_ds = regimes["dense"]["test"]
    print(f"  Dataset: {meta['dataset_name']} | Task: {meta['task_name']}")
    print(f"  Test patients: {len(test_ds)} | Features: {meta['n_features']}")

    print("\n── Loading checkpoints ─────────────────────────────────────────")
    methods = {}
    for model_name in selected_models:
        methods[model_name] = load_model_checkpoint(model_name, cfg, meta, device, out_dir)
        print(f"  Loaded {model_name}")

    print("\n── Running controlled perturbation tests ───────────────────────")
    perturb_dir = out_dir / "perturbation_test"
    git_meta = collect_git_metadata(Path(__file__).resolve().parent.parent)
    provenance_dir = out_dir / "provenance" / "perturbation_test"
    write_provenance_files(
        provenance_dir,
        {
            "config.json": vars(cfg),
            "git.json": git_meta,
            "git_status.txt": git_meta.get("git_status_short") or "",
            "git_diff.patch": git_meta.get("git_diff") or "",
            "git_diff_cached.patch": git_meta.get("git_diff_cached") or "",
            "data.json": build_data_provenance(cfg, meta, split_name="test"),
            "loaded_checkpoints.json": {
                model_name: str(_checkpoint_for(model_name, out_dir).resolve())
                for model_name in selected_models
            },
        },
    )

    run_name = cfg.run_name or default_run_name("perturbation_test", cfg, selected_models)
    with start_run_if_enabled(
        cfg.track,
        cfg.experiment_name,
        run_name,
        tracking_uri=cfg.tracking_uri,
    ) as mlflow:
        if mlflow is not None:
            set_common_tags(mlflow, "perturbation_test", cfg, {"component_count": len(selected_models)})
            log_git_tags(mlflow, git_meta)
            log_params_from_namespace(mlflow, cfg, exclude={"run_name", "experiment_name", "tracking_uri", "track"})
            mlflow.log_param("results_dir_resolved", str(out_dir.resolve()))
            mlflow.log_param("run_root_resolved", str(out_dir.resolve()))
            mlflow.log_param("output_dir_resolved", str(perturb_dir.resolve()))
            maybe_log_artifacts(mlflow, provenance_dir, artifact_path="provenance")

        results = run_all_perturbation_tests(methods, test_ds, device, cfg)
        save_perturbation_outputs(results, perturb_dir)

        metrics = {}
        for model_name, model_result in results.items():
            sparsity = model_result.get("sparsity_response", {}).get("by_retention", {})
            full_retention = sparsity.get("1.0", {})
            metrics[f"{model_name}_auc_retention_1_0"] = full_retention.get("auc")
            metrics[f"{model_name}_ece_retention_1_0"] = full_retention.get("ece")
        maybe_log_metrics(mlflow, metrics)
        maybe_log_artifacts(mlflow, perturb_dir, artifact_path="outputs/perturbation_test")

    print(f"\nPerturbation-test outputs saved to {perturb_dir}")
    print(f"  {perturb_dir / 'summary.md'}")
    print(f"  {perturb_dir / 'perturbation_test_results.json'}")
    print("Done.")


if __name__ == "__main__":
    main()
